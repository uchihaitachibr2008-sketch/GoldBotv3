...
class Compras(commands.Cog):
    ...
    async def comprar(self, interaction: discord.Interaction, valor: float):
        if valor <= 0:
        ...
        payment = sdk.payment().create({
            ...
            "payer": {"email": "comprador@discord.com"}
        })
        ...
        async with pool.acquire() as conn:
            ...
            """, interaction.user.id, payment_id, valor, moedas)
        ...
        await interaction.response.send_message(
            f"🛒 Compra criada em {canal.mention}", ephemeral=True
        )
    ...
    async def verificar_pagamentos(self):
        ...
        for compra in compras:
            ...
            if status == "approved":
                ...
                async with pool.acquire() as conn:
                    await conn.execute("""
                        UPDATE compras SET status = 'aprovado'
                        WHERE payment_id = $1
                    """, compra["payment_id"])
                ...
                canal = discord.utils.get(
                    guild.text_channels,
                    name=f"compra-{self.bot.get_user(compra['user_id']).name}"
                )
                ...
                hist = self.bot.get_channel(HISTORICO_COMPRAS)
                if hist:
                    await hist.send(
                        f"🟢 **Compra confirmada**\n"
